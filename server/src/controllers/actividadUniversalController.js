// controllers/actividadUniversalController.js
import { ActividadPaciente } from "../models/actividadPacienteModel.js";

export const registrarActividadUniversal = async (req, res) => {
  try {
    const {
      id_paciente,
      id_actividad,
      estado,
      evidencia_texto,
      duracion_segundos,
    } = req.body;

    if (!id_paciente || !id_actividad) {
      return res.status(400).json({
        success: false,
        error: "id_paciente e id_actividad son requeridos"
      });
    }

    const evidencia_foto = req.file
      ? `/uploads/evidencias/${req.file.filename}`
      : null;

    const result = await ActividadPaciente.registrar({
      id_paciente,
      id_actividad,
      estado: estado || "completada",
      evidencia_texto: evidencia_texto || null,
      evidencia_foto,
      duracion_segundos: duracion_segundos || null
    });

    return res.status(201).json({
      success: true,
      id_actividad_paciente: result.id,
      evidencia_foto
    });

  } catch (error) {
    console.error("Error registrar actividad universal:", error);
    return res.status(500).json({
      success: false,
      error: "Error al registrar actividad del paciente"
    });
  }
};


export const getActividadesPaciente = async (req, res) => {
  try {
    const { id_paciente } = req.params;
    const registros = await ActividadPaciente.getByPaciente(id_paciente);

    return res.status(200).json({
      success: true,
      registros
    });
  } catch (e) {
    return res.status(500).json({ success: false });
  }
};
